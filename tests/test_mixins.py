...
class TestBaseMixin(unittest.TestCase):
    ...
    def test_name_default_generation(self):
        ...
        assert all(
            char.upper() in UniqueNameGenerator.VOCAB for char in model.name_compass
        )
    ...
    def test_name_validator_reject_invalid_name(self):
        with pytest.raises(
            ValidationError, match=r"The character `\^` is not allowed as `name`."
        ):
...
class TestBaseMixinExport(unittest.TestCase):
    ...
    def test_to_json_basic(self):
        ...
        expected_json = json.dumps(
            {"id": 1, "name_compass": "Test"}, indent=4, sort_keys=True
        )
    ...
    def test_to_json_with_special_characters(self):
        ...
        expected_json = json.dumps(
            {"id": 4, "name_compass": "NameWith!Mark"}, indent=4, sort_keys=True
        )
    ...
    def test_to_json_sort_keys(self):
        ...
        expected_json = json.dumps(
            {"id": 5, "name_compass": "Zebra"}, indent=4, sort_keys=True
        )
...
class TestAutoIdModelMixin(unittest.TestCase):
    ...
    def test_validate_unique_id_invalid_type(self):
        with pytest.raises(
            ValidationError, match=r"invalid literal for int\(\) with base 10"
        ):
...
class TestNameIdModelMixin(unittest.TestCase):
    ...
    def test_name_registration_with_retries(self):
        ...
        with pytest.raises(
            ValidationError, match=f"Value `{name}` has already been registred."
        ):
